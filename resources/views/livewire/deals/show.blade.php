<div data-testid="deal-detail">
    <x-ui.page-header :title="$deal->title" :subtitle="'Stage: '.$deal->stage?->name">
        <x-slot:actions>
            @if ($deal->stage?->slug !== \App\Models\PipelineStage::WON && $deal->stage?->slug !== \App\Models\PipelineStage::LOST)
                <x-ui.button type="button" variant="danger" wire:click="openLostModal" data-testid="open-lost-modal">Mark lost</x-ui.button>
            @endif
        </x-slot:actions>
    </x-ui.page-header>
        ...
        <section class="lg:col-span-2 space-y-4">
            <x-ui.card title="Deal" data-testid="deal-header-section">
                @if ($this->canEdit())
                    <form wire:submit="save" class="grid gap-4 sm:grid-cols-2" data-testid="deal-edit-form">
                        <x-ui.input wire:model="title" name="title" label="Title" :error="$errors->first('title')" />
                        <x-ui.input wire:model="value" name="value" type="number" min="0" step="0.01" label="Value" :error="$errors->first('value')" />
                        <div class="sm:col-span-2 flex justify-end">
                            <x-ui.button type="submit" data-testid="save-deal">Save</x-ui.button>
                        </div>
                    </form>
                @else
                    ...
                    @if ($deal->won_at)
                        <p class="mt-3 text-xs text-success" data-testid="deal-locked-notice">Won deal — read-only.</p>
                    @endif
                @endif
            </x-ui.card>
            ...
            <x-ui.card title="Lead" data-testid="lead-info-section">
                <dl class="grid gap-2 text-sm">
                    <div><dt class="text-ink-muted">Name</dt><dd>{{ $deal->lead?->name }}</dd></div>
                    <div><dt class="text-ink-muted">Email</dt><dd>{{ $deal->lead?->email }}</dd></div>
                    <div><dt class="text-ink-muted">Phone</dt><dd>{{ $deal->lead?->phone ?? '—' }}</dd></div>
                </dl>
            </x-ui.card>

            <x-ui.card title="Notes" data-testid="notes-section">
                <livewire:deals.notes-panel :deal="$deal" :wire:key="'notes-'.$deal->id" />
            </x-ui.card>

            <x-ui.card title="Activity" data-testid="activity-section">
                <livewire:deals.timeline :deal="$deal" :wire:key="'timeline-'.$deal->id" />
            </x-ui.card>
        </section>

        <aside class="space-y-4">
            <x-ui.card title="Owner" data-testid="owner-section">
            ...
            <x-ui.card title="Chat" data-testid="chat-shortcut-section">
                ...
                @if ($isWhatsappConnected && $hasPhone)
                    <a href="{{ route('whatsapp.conversation', ['lead' => $deal->lead_id, 'deal' => $deal->id]) }}" class="text-sm text-primary-500" data-testid="chat-link">Open WhatsApp chat</a>
                @elseif (! $isWhatsappConnected)
                    <p class="text-xs text-ink-muted" data-testid="chat-blocked-disconnected">WhatsApp not connected.</p>
                @else
                    <p class="text-xs text-ink-muted" data-testid="chat-blocked-no-phone">No phone on lead.</p>
                @endif
            </x-ui.card>
        </aside>
    </div>
